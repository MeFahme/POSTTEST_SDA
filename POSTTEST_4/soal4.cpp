...
void sortedInsert(Node *&head_ref, int data) {
    ...
    if (data < head_ref->data) {
        Node* tail = head_ref->prev; // Dapatkan node terakhir
        newNode->next = head_ref;    // New node menunjuk ke head
        newNode->prev = tail;        // New node menunjuk ke tail
        tail->next = newNode;        // Tail menunjuk ke new node
        head_ref->prev = newNode;    // Head menunjuk ke new node
        head_ref = newNode;          // Update head_ref ke new node
        return;
    }
