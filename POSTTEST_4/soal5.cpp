...
void exchangeHeadAndTail(Node *&head_ref) {
    ...
    if (head->next == tail) {   // Buat kondisi jika hanya 2 node, cukup swap head_ref
        head_ref = tail;        // Simpan neighbor ( yaitu head_next dan tail_prev)
        head->next = head_ref;  // Update koneksi: tail_prev <-> tail <-> head_next
        tail->prev = head_ref;  // Update koneksi: head_next <- ... -> tail_prev <-> head <-> tail_prev
        head_ref->prev = tail;  // terakhir Update head_ref
        tail->next = head;      
        return;
    }
